...
import { FileUp, Plus, Settings, Trash2, Upload, Activity, Save, X, Edit3, Bold, Italic, Code, List, Image as ImageIcon, Link as LinkIcon, Paperclip, Loader2, FileText, ArrowUp, ArrowDown, AlignLeft, Heading, Minus, Video, LayoutGrid, AlertCircle, Link, ListOrdered, Quote, Copy } from 'lucide-react';
...
export const TutorialEditor: React.FC<TutorialEditorProps> = ({ tutorials, refreshTutorials, loading }) => {
      ...
      <div key={block.id} className="border border-hw-blue/20 bg-hw-blue/5 rounded-sm overflow-hidden mb-4 group">
        <div className="flex items-center justify-between px-3 py-2 bg-hw-blue/10 border-b border-hw-blue/20">
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => copyBlockContent(block)} className="p-1 text-hw-blue/40 hover:text-hw-blue" title="Copy Content"><Copy className="w-3 h-3" /></button>
          ...
          {block.type === 'markdown' && (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-1 border-b border-hw-blue/10 pb-2 mb-2">
                ...
                <button onMouseDown={e => e.preventDefault()} onClick={() => execCommand('strikeThrough')} className="p-1.5 hover:bg-hw-blue/10 rounded text-hw-blue/60 hover:text-hw-blue transition-colors" title="Strikethrough"><Minus className="w-3 h-3" /></button>
                <div className="w-px h-3 bg-hw-blue/10 mx-1" />
  ...
  return (
    <div className="space-y-8 flex flex-col h-full relative">
      <div className="flex items-center justify-between border-b border-hw-blue/20 pb-4 shrink-0">
        <div className="flex items-center gap-4">
          <Settings className="w-6 h-6 text-hw-blue" />
          <div>
            <h2 className="text-2xl font-black tracking-tighter uppercase hw-glow">Documentation Editor</h2>
            <p className="text-[10px] text-hw-blue/40 uppercase tracking-[0.2em]">Manage tutorials and knowledge base</p>
          </div>
        </div>
        <button
          onClick={() => {
          ...
          className="flex items-center gap-2 bg-hw-blue text-hw-black px-4 py-2 rounded-sm text-[10px] font-bold uppercase tracking-wider hover:bg-hw-blue/80 transition-all"
        >
      ...
      <div className="flex-1 overflow-hidden">
        {showAddForm ? (
          <div className="hw-panel p-0 h-full flex flex-col">
