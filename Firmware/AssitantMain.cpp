...
void setup() {
  ...
  // Initialize Hardware Serial 1 on pins 20/21
  // Standard baud is usually 9600 or 115200; adjust if your external device differs
  ...
  Serial.println("\n--- SYSTEM READY ---");
  Serial.println("- Bridge Active: Serial <-> Serial1 (Pins 20/21)");
}

// --- YOUR ORIGINAL RFID FUNCTIONS ---
...
// --- YOUR ORIGINAL EEPROM FUNCTIONS ---
...
void loop() {
  // 1. BRIDGE: Pass data from Pins 20/21 to the USB Console
  while (Serial1.available()) {
    Serial.write(Serial1.read());
  }

  // 2. BRIDGE: Pass data from USB Console to Pins 20/21 
  // (Only if it's not a recognized local command)
  if (Serial.available()) {
    String input = Serial.readStringUntil('\n');
    input.trim();
    ...
    if (input.startsWith("DUMP 0x")) {
      ...
      // If the command isn't for the ESP, send it to the device on pins 20/21
